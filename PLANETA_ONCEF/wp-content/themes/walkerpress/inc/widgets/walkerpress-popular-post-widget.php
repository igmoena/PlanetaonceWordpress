<?php /**
	*
	*Widgets for latest post
	*/
	class WalkerPress_Popular_Blog_Widget extends WP_Widget {
	public function __construct() {
		parent::__construct(
			'walkerpress_popular_blog', // Base ID
			__( 'WalkerPress: Popular Post', 'walkerpress' ), // Name
			array( 'description' => __( 'Popular post Widets for the theme.', 'walkerpress' ), ) // Args
		);
	}
	/**
	...
	public function widget( $args, $instance ) {
		echo '<div class="sidebar-widget-block popular-post">';
			extract( $args );
			$title    = isset( $instance['title'] ) ? apply_filters( 'widget_title', $instance['title'] ) : '';
			if ( $title ) {
				echo '<h5 class="walkerpress-custom-header"> <span>'.  apply_filters( 'widget_title', $instance['title'] ) .'</span></h5>';
			}
			$popular_post_layout = isset( $instance['popular_post_layout'] ) ? apply_filters( 'popular_post_layout', $instance['popular_post_layout'] ) : 'layout-list';
			...
			$wp_query = new WP_Query( $popular_posts_args );?>
			<ul class="popular-blog-widget <?php echo esc_attr($popular_post_layout);?>">
			<?php while ($wp_query->have_posts() ) : $wp_query->the_post(); ?>
		    
			     <li class="post-box popular-posts">
	...
	*/
		public function form( $instance ) {
			...
			$defaults = array(
				'title'    => '',
				'popular_post_layout' => 'layout-list',
			...
			<div class="walkerpress-widget-form">
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'popular_post_layout' ) ); ?>"><?php _e( 'Choose Layout', 'walkerpress' ); ?></label><br />
	                <input type="radio" value="layout-list" name="<?php echo $this->get_field_name( 'popular_post_layout' ); ?>" <?php checked( $popular_post_layout, 'layout-list' ); ?> id="<?php echo $this->get_field_id( 'popular_post_layout' ); ?>" />
	                <?php esc_attr_e( 'List Layout', 'walkerpress' ); ?>
	                <input type="radio" value="layout-thumb" name="<?php echo $this->get_field_name( 'popular_post_layout' ); ?>" <?php checked( $popular_post_layout, 'layout-thumb' ); ?> id="<?php echo $this->get_field_id( 'popular_post_layout' ); ?>" />
		...
		*/
		public function update( $new_instance, $old_instance ) {
			...
			$instance['popular_post_layout']    = isset( $new_instance['popular_post_layout'] ) ? wp_strip_all_tags( $new_instance['popular_post_layout'] ) : 'layout-list';
...
function walkerpress_register_popular_blog_widget() {
	register_widget( 'WalkerPress_Popular_Blog_Widget' );
}
add_action( 'widgets_init', 'walkerpress_register_popular_blog_widget' );	